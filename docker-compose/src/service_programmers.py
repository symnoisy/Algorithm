...
class Greedy:
    ...
    def gymSuit(self, n, lost, reserve):
        '''
        체육복
        ...
        #학생별로 체육복을 빌려줄 수 있는 학생에 대한 경우의 수
        for rClothe in reserve:
            case[rClothe] = [rClothe-1, rClothe+1]
        available = list(case.values())


        print(available)
        while len(lost) > 0:
            for idxAvail, valAvail in enumerate(available):
                if lost == valAvail[1]:
                    result += 1
                    available.pop(idxAvail)
                elif lost != valAvail[1] and lost == valAvail[0]:
                    result += 1
                    available.pop(idxAvail)
            lost.pop(0)
        return result
...
if __name__ == '__main__':
    ...
    '''
    greedy = Greedy()
    result = greedy.gymSuit(5,[2, 4],[1, 3, 5])
    print(result)
    # result = greedy.gymSuit(5, [2, 4], [3])
    # print(result)
    # result = greedy.gymSuit(3, [3], [1])
    # print(result)